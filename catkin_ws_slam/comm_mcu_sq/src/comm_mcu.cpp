...
struct timeval radar_begin_tm;
struct timeval radar_end_tm;

int radar_timedelay = 0; //100ms radar time delat

#define __PUBLISH_SLAM_LOCATION__ 0
...
static void printtime(timeval radar_bedgin_tm, char namestring[20], int radar_posi[3], int GPS_posi[3]);
...
//cheng add 19/3/4
McuSendDada *McuSendDada_Ptr = (McuSendDada *)malloc(sizeof(McuSendDada));
int Car_Speed_Flag;
PCWRITEDATA *PCWRITEDATA_Ptr = (PCWRITEDATA *)malloc(sizeof(PCWRITEDATA));
...
void publish_mcu(PCREADDATA*& data, vector< vector<int> >& worldlots, ros::Publisher& pub_mcu)
{
    ...
    printf("cheng debug: trail_arc_radius = %d \n", data->trail_arc_radius);
    printf("cheng debug: trail_arc_angle = %d \n", data->trail_arc_angle);

printf("cheng debug: Steering_Wheel_angle = %d \n", data->Steering_Wheel_angle);
printf("cheng debug: CarSpeed = %d \n", data->CarSpeed);

printf("cheng debug: Timer_Couter = %d \n", data->Timer_Couter);
printf("cheng debug: gear_status_pre = %d \n", data->gear_status_pre);
printf("cheng debug: switchCannelnum = %d \n", data->switchCannelnum);

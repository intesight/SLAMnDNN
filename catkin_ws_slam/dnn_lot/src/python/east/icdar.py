...
tf.app.flags.DEFINE_string('training_data_path', './data/ch4_training_images/',##'/data/ocr/icdar2015/',
...
def restore_rectangle_rbox(origin, geometry):
    ...
    # for angle < 0
    origin_1 = origin[angle < 0]#(560, 2)
    d_1 = d[angle < 0]          #(560, 4)
    angle_1 = angle[angle < 0]  #(n,)
    if origin_1.shape[0] > 0:#origin_1的第一维长度
        # (10,560)
        ...
        # p(d1,d1)
        p = p.transpose((1, 0)).reshape((-1, 5, 2))  # N*5*2  (560, 5, 2)

        rotate_matrix_x = np.array([np.cos(-angle_1), -np.sin(-angle_1)]).transpose((1, 0))#(560,2)
        #(560,5,2)
        rotate_matrix_x = np.repeat(rotate_matrix_x, 5, axis=1).reshape(-1, 2, 5).transpose((0, 2, 1))  # N*5*2

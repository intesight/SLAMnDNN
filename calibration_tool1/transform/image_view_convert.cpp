...
//float stretch_coef;  //xml  平行四边形拉伸系数
...
int undistort_cylinder_image(IN Mat raw_image, INOUT Mat column_image, IN float fov[2],IN VIEW_E view_index)
{
    ...
    FILE* fp_lut;
//    float pose_f[3] = {45.242599487, 4327.087890625, 658.926025391};  //GL8_front_pose
//    float pose_r[3] = {900.442443848,        2260.262695312,        1076.897216797};  //GL8_right_pose
...
int world2pixel(IN double world_position[3], IN double trans[3][4], INOUT float pixel_position[2], IN VIEW_E view_index)
{
    ...
    if(xml_version)
    {
        cam_world_point[0] = trans[0][0] * world_position[0] + trans[0][1] * world_position[1] + trans[0][2] * world_position[2] + trans[0][3];
        cam_world_point[1] = trans[1][0] * world_position[0] + trans[1][1] * world_position[1] + trans[1][2] * world_position[2] + trans[1][3];
...
{
    cout<<"enter the dir : birdview_image_generate"<<endl;
    double trans[3][4] = {0};
    int src_width = raw_image.cols;
    int src_height= raw_image.rows;
    int image_size[2] = {0};
...
//    float varN = 20, varG = 0.05;
   ...
   // double t = (double)cvGetTickCount();
    ...
    string lut_file_name_b0 = run_dir + "birdview_front.bin";
        fp_lut_birdview_front = fopen(lut_file_name_b0.c_str(),"rb");
    string lut_file_name_b1 = run_dir + "birdview_rear.bin";
        fp_lut_birdview_rear = fopen(lut_file_name_b1.c_str(),"rb");
    string lut_file_name_b2 = run_dir + "birdview_left.bin";
        fp_lut_birdview_left = fopen(lut_file_name_b2.c_str(),"rb");
    string lut_file_name_b3 = run_dir + "birdview_right.bin";
        fp_lut_birdview_right = fopen(lut_file_name_b3.c_str(),"rb");
    ...
    file_data_front = (uint32_t *) malloc(front_width * front_height * (sizeof(int) * 2 + 4 * sizeof(float)));
    fread(file_data_front, front_width * front_height * (sizeof(int) * 2 + 4 * sizeof(float)), 1, fp_lut_birdview_front);

    file_data_right = (uint32_t *) malloc(right_width * right_height * (sizeof(int) * 2 + 4 * sizeof(float)));
    fread(file_data_right, right_width * right_height * (sizeof(int) * 2 + 4 * sizeof(float)), 1, fp_lut_birdview_right);

    file_data_rear = (uint32_t *) malloc(rear_width * rear_height * (sizeof(int) * 2 + 4 * sizeof(float)));
    fread(file_data_rear, rear_width * rear_height * (sizeof(int) * 2 + 4 * sizeof(float)), 1, fp_lut_birdview_rear);

    file_data_left = (uint32_t *) malloc(left_width * left_height * (sizeof(int) * 2 + 4 * sizeof(float)));
    fread(file_data_left, left_width * left_height * (sizeof(int) * 2 + 4 * sizeof(float)), 1, fp_lut_birdview_left);
